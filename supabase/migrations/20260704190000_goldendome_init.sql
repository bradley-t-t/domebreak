...
create table if not exists public.gd_matches (
  ...
  status text not null default 'lobby' check (status in ('lobby','build','combat','done')),
  ...
  created_by uuid references public.gd_players(id) on delete set null,
  winner_player_id uuid references public.gd_players(id) on delete set null,
  created_at timestamptz not null default now()
);
...
create table if not exists public.gd_match_players (
  match_id uuid not null references public.gd_matches(id) on delete cascade,
  player_id uuid not null references public.gd_players(id) on delete cascade,
  slot int not null,
  handle text not null,
...
create table if not exists public.gd_placements (
  ...
  match_id uuid not null references public.gd_matches(id) on delete cascade,
  player_id uuid not null references public.gd_players(id) on delete cascade,
  kind text not null check (kind in ('silo','interceptor','radar','dome')),
  lng double precision not null,
  lat double precision not null,
  target_city_id uuid references public.gd_cities(id) on delete set null,
...
create table if not exists public.gd_results (
  match_id uuid primary key references public.gd_matches(id) on delete cascade,
  winner_player_id uuid references public.gd_players(id) on delete set null,
...
drop policy if exists gd_matches_read on public.gd_matches;
create policy gd_matches_read on public.gd_matches for select using (true);
drop policy if exists gd_match_players_read on public.gd_match_players;
create policy gd_match_players_read on public.gd_match_players for select using (true);
drop policy if exists gd_cities_read on public.gd_cities;
create policy gd_cities_read on public.gd_cities for select using (true);
drop policy if exists gd_results_read on public.gd_results;
create policy gd_results_read on public.gd_results for select using (true);
...
do $$
begin
  begin execute 'alter publication supabase_realtime add table public.gd_matches'; exception when duplicate_object then null; end;
  begin execute 'alter publication supabase_realtime add table public.gd_match_players'; exception when duplicate_object then null; end;
  begin execute 'alter publication supabase_realtime add table public.gd_cities'; exception when duplicate_object then null; end;
  begin execute 'alter publication supabase_realtime add table public.gd_results'; exception when duplicate_object then null; end;
end$$;
