...
const HOMES = [
  { name: "United States", cities: [["Washington", -77.04, 38.9], ["New York", -74.0, 40.71], ["Los Angeles", -118.24, 34.05]] },
  { name: "Russia", cities: [["Moscow", 37.62, 55.75], ["St Petersburg", 30.31, 59.94], ["Novosibirsk", 82.92, 55.03]] },
  ...
  { name: "United Kingdom", cities: [["London", -0.13, 51.51], ["Manchester", -2.24, 53.48], ["Birmingham", -1.9, 52.48]] },
  ...
  { name: "Australia", cities: [["Canberra", 149.13, -35.28], ["Sydney", 151.21, -33.87], ["Melbourne", 144.96, -37.81]] },
  { name: "Egypt", cities: [["Cairo", 31.24, 30.04], ["Alexandria", 29.92, 31.2], ["Luxor", 32.64, 25.69]] },
  { name: "South Africa", cities: [["Pretoria", 28.19, -25.75], ["Johannesburg", 28.05, -26.2], ["Cape Town", 18.42, -33.92]] },
  { name: "Iran", cities: [["Tehran", 51.39, 35.69], ["Mashhad", 59.61, 36.3], ["Isfahan", 51.68, 32.65]] },
  { name: "Mexico", cities: [["Mexico City", -99.13, 19.43], ["Guadalajara", -103.35, 20.66], ["Monterrey", -100.32, 25.69]] },
...
async function playAi(participant: any, cities: any[], seedBase: number) {
  ...
  const buy = (kind: string, lng: number, lat: number, target?: string) => {
    ...
    rows.push({ match_id: participant.match_id, player_id: participant.player_id, kind, lng, lat,
...
async function resolve(matchId: string) {
  ...
  for (const silo of silos) {
    ...
    if (rand() < 1 - survival) {
      ...
      replay.push({ t: t + 0.9, type: destroyed ? "destroy" : "hit",
        toLng: target.lng, toLat: target.lat, cityId: target.id, damage: WARHEAD_DAMAGE, attackerSlot: slotOf[silo.player_id] });
    }
  }
  ...
  for (const mp of players!) { score[mp.slot] = 0; alive[mp.slot] = 0; }
  ...
  for (const mp of players!) {
    ...
    if (s > best) { best = s; winnerSlot = mp.slot; tie = false; }
    else if (s === best) tie = true;
  }
  if (tie) {
    let bestAlive = -1; winnerSlot = null; let atie = false;
    for (const mp of players!) {
      if (score[mp.slot] !== best) continue;
      if (alive[mp.slot] > bestAlive) { bestAlive = alive[mp.slot]; winnerSlot = mp.slot; atie = false; }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (a === "start") {
      ...
      const { count } = await db.from("gd_match_players").select("*", { count: "exact", head: true }).eq("match_id", match.id);
