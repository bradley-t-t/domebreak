...
> "Day-one patch prep is for Release-stage projects. Current stage: [stage]. This skill is not appropriate until you are
> approaching launch."
...
> "⚠️ Patch scope is [N hours] — this exceeds a safe day-one window. Consider deferring lower-priority items to patch
> 1.1. A bloated day-one patch introduces more risk than it removes."
...
For config/data-only fixes: make the change directly (no programmer agent needed). Confirm the value changed and re-run
any relevant smoke test.

---

## Phase 5: Patch QA Gate

This is a lightweight QA pass — not a full `/team-qa`. The patch is already QA-approved from the release gate; we are
only re-verifying the changed areas.
...
Ask qa-lead to determine: **Is a targeted smoke check sufficient, or do any fixes touch systems that require a broader
regression?**
...
QA verdict must be PASS or PASS WITH WARNINGS before proceeding. If FAIL: scope the failing fix out of the day-one patch
and defer to 1.1.
...
> "⚠️ S1 bugs remain open and were not patched. These are accepted risks. Document them in the rollback plan trigger
