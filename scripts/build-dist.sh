#!/usr/bin/env bash
# Build distributable DomeBreak installers for macOS and Windows and collect
...
"${SSH[@]}" "$WIN_HOST" 'powershell -NoProfile -Command "cd '"$WIN_REPO"'; $e = Get-ChildItem release\*.exe | Sort-Object LastWriteTime | Select-Object -Last 1; Copy-Item $e.FullName $env:USERPROFILE\DomeBreak-Setup.exe -Force; Write-Output $e.Name"'
"${SCP[@]}" "$WIN_HOST:DomeBreak-Setup.exe" "$OUT/DomeBreak-Setup.exe"
