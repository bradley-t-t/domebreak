...
export default function ContextMenu({ x, y, title, items, onClose }) {
  return (
    <>
      <div className="gd-ctx-catcher" onClick={onClose} onContextMenu={(e) => { e.preventDefault(); onClose(); }} />
      <div className="gd-ctx" style={{ left: Math.min(x, window.innerWidth - 210), top: Math.min(y, window.innerHeight - 40 - items.length * 34) }}>
