  ...
  if (collapsed) {
    return (
      <div className="gd-console-rail">
        {TABS.map((t) => <button key={t.id} className={`gd-rail-tab ${active === t.id ? "active" : ""}`} title={t.label} onClick={() => { setActive(t.id); setCollapsed(false); }}>{t.glyph}</button>)}
      </div>
    );
  }
  return (
    <div className="gd-console">
      <div className="gd-tabs">
        {TABS.map((t) => <button key={t.id} className={`gd-tab ${active === t.id ? "active" : ""}`} onClick={() => setActive(t.id)}><span className="gd-tab-glyph">{t.glyph}</span>{t.label}</button>)}
        <button className="gd-collapse" title="Collapse" onClick={() => setCollapsed(true)}>◀</button>
      </div>
      <div className="gd-panel" key={active}>
        {active === "units" && <UnitsPanel world={world} mySlot={mySlot} placing={placing} setPlacing={setPlacing} />}
