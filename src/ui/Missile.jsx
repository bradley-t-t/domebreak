...
export default function Missile({ p }) {
  ...
  return (
    <Marker longitude={p.lng} latitude={p.lat} anchor="center" offset={[0, -(p.altNorm || 0) * (ALT[p.type] || 60)]}>
      <div className="gd-missile" style={{ transform: `rotate(${deg}deg)` }}>
        <span className="gd-missile-glow" /><span className="gd-missile-body" /><span className="gd-missile-flame" />
      </div>
    </Marker>
  );
}
