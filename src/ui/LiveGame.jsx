...
export default function LiveGame({ world, globe, onToggleGlobe, onPause, backdrop }) {
  ...
  }, [menu, details, placing, attackMode, onPause]);

  const backdropFC = useMemo(() => ({
    type: "FeatureCollection",
    features: (backdrop || []).map((c) => ({ type: "Feature", properties: { cap: c.cap ? 1 : 0 }, geometry: { type: "Point", coordinates: [c.lng, c.lat] } })),
  }), [backdrop]);
  ...
  const liveFC = useMemo(() => ({
    ...
    features: w.cities.map((c) => ({
      type: "Feature",
      properties: { id: c.id, cap: c.cap ? 1 : 0, mine: c.slot === mySlot ? 1 : 0, color: c.alive ? SLOT_COLOR[c.slot] : "#3a3a3a" },
  ...
  const ranges = useMemo(() => {
    ...
    for (const u of w.units) {
      ...
      if (def.kind === "defense") { const linked = w.units.some((r) => r.type === "radar" && r.slot === mySlot && r.hp > 0 && circleDist(r, u) <= UNITS.radar.range); radius = linked ? def.range * 2.5 : def.range; }
    ...
    if (placing && cursor && UNITS[placing].kind !== "offense" && UNITS[placing].range <= 4000) {
      const c = circle(cursor.lng, cursor.lat, UNITS[placing].range); c.properties = { color: "#f4c02a", sel: 1, radar: UNITS[placing].kind === "support" ? 1 : 0 }; f.push(c);
    }
  ...
  const cmdLines = useMemo(() => ({
    ...
    features: w.units.filter((u) => u.slot === mySlot && u.targetId).map((u) => {
      ...
      return t ? { type: "Feature", properties: {}, geometry: { type: "LineString", coordinates: gcTrail(u.lng, u.lat, t.lng, t.lat, 1, 18) } } : null;
    }).filter(Boolean),
  }), [w.units, w.time, mySlot]);

  const trails = useMemo(() => ({
    type: "FeatureCollection",
    features: w.projectiles.map((p) => ({ type: "Feature", properties: {}, geometry: { type: "LineString", coordinates: gcTrail(p.fromLng, p.fromLat, p.toLng, p.toLat, p.progress) } })),
  }), [w.projectiles, w.time]);

  const intTrails = useMemo(() => ({
    type: "FeatureCollection",
    features: w.interceptors.map((it) => ({ type: "Feature", properties: {}, geometry: { type: "LineString", coordinates: [[it.fromLng, it.fromLat], [it.lng, it.lat]] } })),
  }), [w.interceptors, w.time]);
  ...
  const onCtx = (e) => {
    const feat = e.features?.find((f) => f.layer.id === "live-cities");
    if (feat) openCityMenu(feat.properties.id, e.originalEvent);
    else setMenu(null);
  };

  const addPin = (type, ent) => {
    const key = `${type}-${ent.id}`;
    setPins((p) => p.some((x) => x.key === key) ? p : [...p, { key, type, id: ent.id, label: type === "city" ? ent.name : unitLabel(ent.type, ent.slot), lng: ent.lng, lat: ent.lat, color: SLOT_COLOR[ent.slot] }]);
  };
  const goPin = (p) => mapRef.current?.flyTo?.({ center: [p.lng, p.lat], zoom: 4, duration: 800 });

  const openCityMenu = (id, ev) => {
    const c = w.cities.find((x) => x.id === id); if (!c) return;
    const mine = c.slot === mySlot; const rel = relation(c.slot);
  ...
  const openUnitMenu = (u, ev) => {
    ...
    if (mine) items.push({ label: "Scrap (refund 50%)", danger: true, onClick: () => { api.scrap(u.id); if (selUnit === u.id) setSelUnit(null); } });
  ...
  return (
    <>
      <WorldMap globe={globe} onMap={(m) => (mapRef.current = m)}
        ...
        <Source id="backdrop-src" type="geojson" data={backdropFC}>
          <Layer id="backdrop-cities" type="circle" paint={{ "circle-radius": ["case", ["==", ["get", "cap"], 1], 2.3, 1.3], "circle-color": "#63769a", "circle-opacity": 0.5 }} />
        </Source>
        ...
        <Source id="live-src" type="geojson" data={liveFC}>
          <Layer id="live-cities" type="circle" paint={{ "circle-radius": ["case", ["==", ["get", "cap"], 1], 5, 3], "circle-color": ["get", "color"], "circle-stroke-color": ["case", ["==", ["get", "mine"], 1], "#ffffff", "#05070c"], "circle-stroke-width": ["case", ["==", ["get", "mine"], 1], 1.4, 0.6] }} />
        </Source>
        ...
        {w.units.map((u) => (
          <Marker key={u.id} longitude={u.lng} latitude={u.lat} anchor="center">
            <div className={`gd-unit ${u.slot === mySlot ? "mine" : "enemy"} ${u.id === selUnit ? "sel" : ""}`} title={unitLabel(u.type, u.slot)}
              onClick={(e) => onUnitClick(u, e)} onContextMenu={(e) => openUnitMenu(u, e)}>
      ...
      {!w.over && <Console world={w} api={api} mySlot={mySlot} active={tab} setActive={setTab} placing={placing} setPlacing={(t) => { setPlacing(t); setSelUnit(null); }} />}
      ...
      {selectedCity && !details && !w.over && (
        <div className="gd-selpanel">
          ...
          <div className="gd-selmeta">{nationName(selectedCity.slot)}{selectedCity.state ? " · " + selectedCity.state : ""} · {Math.max(0, Math.round(selectedCity.hp))} hp{selectedCity.slot === mySlot ? " · yours" : " · " + relation(selectedCity.slot)}</div>
          ...
          <div className="gd-card" onClick={(e) => e.stopPropagation()}>
            {details.type === "unit" ? (<>
              ...
              <div className="gd-detail-grid">
                ...
                <div><span>Range</span><b>{UNITS[detailTarget.type].range.toLocaleString()} km</b></div>
              ...
              <div className="gd-detail-grid">
                ...
                <div><span>Relation</span><b>{detailTarget.slot === mySlot ? "you" : relation(detailTarget.slot)}</b></div>
              </div>
            </>)}
