import { SLOT_COLOR, MAX_SLOTS } from "../game/constants.js";

export default function Lobby({ match, players, isHost, meId, onStart, onAddAi, onRemove, onReplaceAi, onSetSlots, busy, error }) {
  const bySlot = {};
          ...
          {slots.map((i) => {
            ...
            return (
              <div key={i} className={`gd-slotrow ${p ? (p.is_ai ? "ai" : "human") : "open"}`}>
                <span className="gd-slot-dot" style={{ background: SLOT_COLOR[i], boxShadow: `0 0 8px ${SLOT_COLOR[i]}` }} />
                ...
                {isHost && (
                  <span className="gd-slotctl">
                    ...
                    {p && p.is_ai && <button className="gd-mini danger" disabled={busy} onClick={() => onRemove(i)}>Remove</button>}
                    {p && !p.is_ai && !isHostSeat && <button className="gd-mini" disabled={busy} onClick={() => onReplaceAi(i)}>→ AI</button>}
                  </span>
                )}
              </div>
            );
