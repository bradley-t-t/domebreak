...
export function useEngine(world) {
  ...
  const api = useMemo(() => ({
    ...
    buyPlace: (type, lng, lat, territoryOk) => { const r = buyPlace(ref.current, ref.current.mySlot, type, lng, lat, territoryOk); force(); return r; },
    ...
    move: (uid, lng, lat, territoryOk) => { const r = moveUnit(ref.current, ref.current.mySlot, uid, lng, lat, territoryOk); force(); return r; },
